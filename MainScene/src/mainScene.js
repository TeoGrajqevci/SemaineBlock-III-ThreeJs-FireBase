...
export default class MainScene {
    constructor() {
        this.loadConfig().then((config) => {
            ...
            this.FirebaseListener = new FirebaseListener(this.buttons, this.onFirebaseData.bind(this));
    ...
    onFirebaseData(key, entry) {
        ...
        if (key == "red" && entry.position == "down") {
            ...
            this.alien = new Alien(this.scene, this.camera, this.renderer, false);
            setTimeout(() => {
              new Alien(this.scene, this.camera, this.renderer, false);
            }, 800);
            setTimeout(() => {
              new Alien(this.scene, this.camera, this.renderer, false);
            }, 1600);
    ...
    initCamera() {
        ...
        const size = 8;
        this.camera = new THREE.OrthographicCamera(-size * aspect, size * aspect, size, -size);
    ...
    loadModel(modelPath) {
        ...
        loader.load(
            ...
            (error) => {
                console.error(`An error occurred while loading the OBJ from ${modelPath}`, error);
            }
        );
    }
